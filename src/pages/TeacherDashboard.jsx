...
export default function TeacherDashboard() {
  ...
  const [showChangeSchoolModal, setShowChangeSchoolModal] = useState(false)
  ...
  const handleChangeSchool = async () => {
    if (!newRegion || !newSchoolName.trim()) {
      return alert("지역과 학교명을 모두 입력해주세요.")
    }
    const newCode = `${newRegion}_${newSchoolName.trim()}`
    try {
      await setDoc(doc(db, 'users', user.uid), { schoolCode: newCode }, { merge: true })
      setSchoolCode(newCode)
      localStorage.setItem('schoolCode', newCode)
      setShowChangeSchoolModal(false)
      alert("학교 정보가 성공적으로 변경되었습니다.")
    } catch (error) {
        ...
        <div className="bg-white p-10 rounded-3xl shadow-xl w-full max-w-md">
          <div className="flex flex-col items-center justify-center gap-4 mb-8">
            <div className="w-16 h-16 rounded-full overflow-hidden shadow-md border-4 border-white shrink-0 bg-white">
              <img src={`${import.meta.env.BASE_URL}teacher_icon.jpg`} alt="선생님" className="w-full h-full object-cover" />
            </div>
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => setShowChangeSchoolModal(true)} className="bg-gray-100 text-gray-700 px-4 py-2 rounded-xl font-bold hover:bg-gray-200 transition-colors shadow-sm">
              정보 변경
            </button>
        ...
        {showChangeSchoolModal && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
            <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl transform transition-all scale-100">
              <h2 className="text-2xl font-bold text-gray-800 mb-6 text-center">학교 정보 변경</h2>
              <div className="flex flex-col gap-4">
                <select 
                  value={newRegion} 
                  onChange={(e) => setNewRegion(e.target.value)}
                  className="border-2 border-gray-100 bg-gray-50 p-3.5 rounded-2xl focus:outline-none focus:border-pastel-blue focus:bg-white text-lg transition-all"
                >
                ...
                <div className="flex gap-3 mt-4">
                  <button 
                    onClick={() => setShowChangeSchoolModal(false)}
                  ...
                  <button 
                    ...
                    className="flex-1 bg-pastel-blue text-gray-800 py-3 rounded-xl font-bold hover:bg-blue-300 transition-colors shadow-md"
                  >
                    변경하기
                  </button>
                </div>
...
function MenuEditor({ schoolCode }) {
  ...
  return (
    <div className="bg-white rounded-[2rem] shadow-xl p-6 md:p-10 border border-gray-100">
      ...
      <div className="w-full max-w-5xl mx-auto bg-gray-200 rounded-[3rem] p-4 md:p-6 shadow-inner relative flex gap-4 md:gap-6 aspect-video">
